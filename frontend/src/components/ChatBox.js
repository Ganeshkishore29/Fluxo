...
const ChatBox = ({ onClose }) => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (res.data.type === "products") {
        setMessages((prev) => [
          ...prev,
          { role: "bot", text: res.data.reply, scrollAnchor: true },
          { role: "products", data: res.data.data },
        ]);
      } else {
        setMessages((prev) => [
          ...prev,
          { role: "bot", text: res.data.data, scrollAnchor: true },
        ]);
      }
    } catch {
      setMessages((prev) => [
  ...
  return (
    <div
       className={`
        fixed z-50 bg-white shadow-xl flex flex-col rounded-xl
        transition-all duration-200

        /*  MOBILE */
        right-2 w-[94vw]
        ${keyboardOpen ? "bottom-0 h-[70vh]" : "bottom-40 h-[360px]"}
      ...
      <div className="flex-1 p-3 overflow-y-auto space-y-3">
        {messages.map((msg, idx) => {
          ...
          return (
            <div
              key={idx}
              ref={msg.scrollAnchor ? scrollTargetRef : null}
